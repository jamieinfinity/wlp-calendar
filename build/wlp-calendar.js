(function (global, factory) {
	...
	(factory((global.wlp_calendar = global.wlp_calendar || {})));
...
var clipCircle = function(radius, delta) {
  ...
  function intersect(a, b, two) {
    ...
    var delta = lambda1 - lambda0,
        polar = abs$1(delta - pi$4) < epsilon$3,
...
var index$5 = function() {
  ...
  pack.padding = function(x) {
    return arguments.length ? (padding = typeof x === "function" ? x : constant$11(+x), pack) : padding;
  };
...
Queue.prototype = queue.prototype = {
  ...
  defer: function(callback) {
    ...
    var t = slice$6.call(arguments, 1);
    t.push(callback);
...
const colorMap = sequential(interpolateBlues);
const numWeeksMax = 53;
const numWeekdaysMax = 6;
const calendarGroupSpacing = 5;
const calendarMargin = {top: 10, right: 10, bottom: 10, left: 100};
const calendarSize = {height: 0, width: 0};
const colorLegendSize = {height: 0, width: 0};
const colorLegendMarginRight = 30;
...
function getDaysDictionary(year) {
    const daysDictionary = {},
        ...
        endDay = new Date("12/31/" + year),
        maxDays = day.count(startDay, endDay) + 1;
    ...
    daySquares.enter().append("rect")
        ...
        .on("click", function (d) {
            const startDate = new Date(d.date + ' 00:00:00'),
                endDate = day.offset(startDate, 1);
    ...
    let monthGroupsNew = monthGroups.enter().append("g")
        .on("click", function (d) {
            const monthDate = new Date(d.name + ' 15 ' + selectedYear),
                startDate = month.floor(monthDate),
                endDate = month.ceil(monthDate);
...
function makeCalendar(domElementID, width, years0, updateSelectedDateSpan) {
    ...
    dayGridXScale = linear().domain([0, numWeeksMax]).range([0, numWeeksMax * dayGridSquareSize]);
    dayGridYScale = linear()
...
function updateFeed(feed) {
    ...
    let years = Object.keys(measurementData['data']).map(d => parseInt(d)),
        numStops = 10,
        colorStops = range(measurementData['measurementMaximum'] * 1.00001, measurementData['measurementMinimum'], -(measurementData['measurementMaximum'] - measurementData['measurementMinimum']) / (numStops - 1.)),
        measurementScale = linear().range([colorLegendSize.height, 0]).domain([measurementData['measurementMinimum'], measurementData['measurementMaximum']]),
